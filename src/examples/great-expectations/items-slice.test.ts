...
it('supports adding an item with the correct name', () => {
  ...
  expect(result).toEqual(
    expect.arrayContaining([expect.objectContaining({ name: 'iPhone' })]),
  );
});
...
it('defaults new items to a packed status of false', () => {
  ...
  expect(result).toEqual([
    expect.objectContaining({ name: 'iPhone', packed: false }),
  ]);
});
...
it('supports removing an item', () => {
  ...
  expect(result).not.toEqual(expect.arrayContaining(state));
});
